...
var FSHADER_SOURCE =
  ...
  ///GLOBALS
var dirLight = false, pointLight = false;
var rArray;
var x, y, nx, ny, n=0;
var flag = true;
...
function main() {
  ...
  if (perstag.checked){
	  ...
	  viewMatrix.setLookAt(0.0, 0, 5, 0, 0, -100, 0, 1, 0);
	  modelMatrix.setRotate(15, 1, 0, 0);  
	  projMatrix.setPerspective(30, canvas.width/canvas.height, 1, 100);
	  
  }
  else if (orthotag.checked) {
  ...
  if (n > 0 && flag) {
	  ...
	  var normtoggle = document.getElementById("toggle").checked;
	  if (normtoggle) { 
	    n = bindNormal(gl);
		bindNormalColor(gl);
...
function mDown(e){
	...
	var orthotag = document.getElementById("orthoToggle");
	if (orthotag.checked) {
		x = 500*((x/1001)*2-1);  //position / canvas.window * 2 - 1 formula for x
		y = 500*(-(y/1001)*2+1); //-position / canvas.window * 2 + 1 formula for y
	} else {
		x = (x/1001)*2-1;
		y = -(y/1001)*2+1;
	}
	...
	// if (rect.left <= tx && tx < rect.right && rect.top <= ty && ty < rect.bottom) {
		// var x_in_canvas = tx-rect.left, y_in_canvas = rect.bottom-ty;
	// }
...
function colorAlgorithm2(){
	...
	var lightVector = new Vector3([1,1,1]);
...
function colorAlgorithm(){
	var temp = new Array();
	var totalClicks = flatVertices.length/216;
...
function normalAlgorithmFlat(ray){
	...
	var normColor = new Array();
	var temp = new Array();
	var totalPoints = flatVertices.length;
	var i=0;
